...
<svelte:head>
	<title>{data.category.label} - Admin</title>
</svelte:head>
	...
	<div class="admin-content">
		<div>
			<div class="admin-content-page">
				<header>
					<h3>
						<a href={resolve("/admin/categories")}><ArrowLeft /></a>
						{data.category.label}
					</h3>
				</header>
				...
				{#if data.success}
					<div class="admin-content-user-content">
						...
						>
							<div>
								<header>
									<h3>Kategorie Daten</h3>
								</header>
								<label for="id">ID (einzigartig)</label>
								<input id="id" type="text" placeholder="ID" name="id" value={data.category.id} />
							...
							{#if form?.success}
								<p style="color: green;">Kategorie erfolgreich aktualisiert</p>
							{:else}
								<p style="color: red;">{form?.message}</p>
							{/if}
							<div>
								<button type="button" onclick={deleteCategory} class="delete"
				...
				{:else}
					<p>Fehler beim Laden der Kategorie: {data.message}</p>
				{/if}
			</div>
		</div>
	</div>
